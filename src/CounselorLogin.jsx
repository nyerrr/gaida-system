...
export default function CounselorLogin() {
  ...
  // Generate captcha when user clicks on antibot field
  ...
  const handleSubmit = async (e) => {
    ...
    if (antibot.toLowerCase() !== captchaText.toLowerCase()) {
      alert("Please enter the correct Antibot Validation code!");
      return;
    }
    ...
    try {
      // Query Supabase for counselor with matching credentials
      ...
      // Store counselor info in localStorage for later use
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-gray-900 p-4 lg:p-8">
        <div className="w-full max-w-md">
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            {/* Faculty ID */}
            <div>
              <label htmlFor="facultyId" className="block text-white text-xs font-medium mb-1">
                Faculty ID
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-colors disabled:opacity-50"
              />
              {/* CAPTCHA Image - Fixed space reserved, captcha left, buttons right */}
                ...
                <div className="flex-shrink-0">
                  {showCaptcha ? (
                    <canvas
                      ...
                      width={150}
                      height={50}
                      className="border border-gray-700 rounded bg-gray-100"
                    />
                  ) : (
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-red-700 hover:bg-red-600 text-white font-semibold py-2 px-4 text-sm rounded-lg transition-colors duration-200 shadow-lg hover:shadow-xl mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
            >
